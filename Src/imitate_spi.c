...
uint8_t ispi1_write_read_byte(uint8_t writeBuffer)
{
  ...
  for (i=7;i>=0;i--) 
  {   
    HAL_GPIO_WritePin(iSPI1_MOSI_GPIO_Port, iSPI1_MOSI_Pin, (GPIO_PinState)(writeBuffer&(1<<i)));//从高位7到低位0进行串行写入 
    //Delay(5);
    iSPI1_CLK_Enable();
    Delay(5);
...
uint8_t ispi3_write_read_byte(uint8_t writeBuffer)
{
  ...
  for (i=7;i>=0;i--) 
  {   
    HAL_GPIO_WritePin(iSPI3_MOSI_GPIO_Port, iSPI3_MOSI_Pin, (GPIO_PinState)(writeBuffer&(1<<i)));//从高位7到低位0进行串行写入
    //Delay(5);
    iSPI3_CLK_Enable();
    Delay(5);
